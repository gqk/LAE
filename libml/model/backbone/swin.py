...
Base on https://github.com/rwightman/pytorch-image-models/blob/v0.5.4/timm/models/swin_transformer.py
"""
# --------------------------------------------------------
# Swin Transformer
...
from ..pet_mixin import AdapterMixin, PrefixMixin
...
class WindowAttention(nn.Module, PrefixMixin, AdapterMixin):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        q = q.reshape(B_, N, self.num_heads, C // self.num_heads)
        q = q.permute(0, 2, 1, 3)

        k = k.reshape(B_, -1, self.num_heads, C // self.num_heads)
        k = k.permute(0, 2, 1, 3)

        v = v.reshape(B_, -1, self.num_heads, C // self.num_heads)
        v = v.permute(0, 2, 1, 3)
...
class SwinTransformerBlock(nn.Module, AdapterMixin):
    ...
    def forward(self, x):
        ...
        attn_windows = self.adapt_module("attn", x_windows, mask=self.attn_mask)
...
def swin_base_patch4_window12_384(pretrained=True, **kwargs):
...
def swin_base_patch4_window7_224(pretrained=True, **kwargs):
...
def swin_large_patch4_window12_384(pretrained=True, **kwargs):
...
def swin_large_patch4_window7_224(pretrained=True, **kwargs):
...
def swin_small_patch4_window7_224(pretrained=True, **kwargs):
...
def swin_base_patch4_window12_384_in22k(pretrained=True, **kwargs):
...
def swin_base_patch4_window7_224_in22k(pretrained=True, **kwargs):
...
def swin_large_patch4_window12_384_in22k(pretrained=True, **kwargs):
...
def swin_large_patch4_window7_224_in22k(pretrained=True, **kwargs):
